...
2. **`{"_tag":"Unauthorized"}`** (HTTP 401): session expired → ask for a fresh `__Host-console_session` cookie value (the legacy `auth=Fe26.2**` seal no longer authenticates). Final — no route retry. HTTP 403 `{"_tag":"Forbidden"}` = restricted/support-only route (final); HTTP 400 `{"_tag":"BadRequest"}` = missing or bad `x-org-id` (final). Cookie paste handling trusts only known names (`__Host-console_session` / `console_session` / `auth`); any other bare value — even with `=` inside — is treated as a session token.
3. **HTTP 5xx** (usually `{"_tag":"InternalServerError"}`): transient — the endpoint is currently intermittent, so the SAME route is retried in place (plasmoid up to 4 attempts, CLI up to 3) and then reported as "Console API temporarily unavailable (HTTP 5xx) — retry shortly". After retries the widget keeps the last known live figures with a "showing last known figures" note instead of blanking the card.
