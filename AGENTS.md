...
- **Credit Header:** Every QML, JavaScript, and configuration source file must include the header:
  `// © Mayanktaker Computers & Web Development | https://mayanktaker.com`
- **Default Endpoint:** OpenCode Go usage data is retrieved from the OpenCode Console JSON API:
- **Default Endpoint:** OpenCode Go usage data comes from the OpenCode Console JSON API — `https://opencode.ai` + a candidate route + `{orgId}/go/status`. Candidate routes (`CONSOLE_STATUS_ROUTES`, first entry is the verified one): `/console/api/internal/orgs/`, `/console/api/orgs/`, `/console/api/v2/orgs/`, `/console/api/v1/orgs/`; suffix `CONSOLE_STATUS_SUFFIX = "/go/status"`. Headers: `Cookie`, `Accept: application/json`, `x-org-id`, `Referer: https://opencode.ai/console/{orgId}/go`. curl runs with `-w 'HTTPSTATUS:%{http_code}'` and the marker is split off the LAST occurrence in stdout. Retry rule: **only HTTP 404 (route moved; empty body) advances to the next candidate route** — 401 (bad session), 200 and network errors are final. When every candidate 404s, plasmoid and CLI surface a "Console API route not found — check for a widget update" message. `bin/opencode-usage` mirrors the same walk. The legacy workspace Go page is retired (302 only). Workspace/org ids look like `wrk_...` or `org_...`.
- **Response Handling:** `api.js` parses the console `go/status` JSON: `access` may be `null` (no Go subscription on the workspace), otherwise read `access.meters.fiveHour` / `week` / `month` (`limitMicroCents`/`usedMicroCents` are BigInts serialized as **strings** — accept string or number). Percentage matches the console exactly: `floor((used*200 + limit) / (limit*2))` clamped to 0..100, 0 when `limit <= 0`. Labels: `fiveHour` → Rolling (5h), `week` → Weekly, `month` → Monthly; the `month` meter has no `resetsAt` (it resets at `access.endsAt`). A dead session returns HTTP 401 with body `{"_tag":"Unauthorized"}` (final — no route retry); a 404 means the API route moved, **not** a bad session.
- **Authentication:** Auth cookies are passed via the `Cookie` header (`auth=<token>`). The server (sst/opencode via iron-session) reads **cookies only** — do not send an `Authorization` header; it is ignored. Iron-session seals begin with `Fe26.2**`, so bare pasted values are auto-prefixed with `auth=`. Sessions expire: copy a fresh value from the console (opencode.ai/console → DevTools → Application → Cookies → opencode.ai → `auth`).
